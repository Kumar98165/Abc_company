...
export function WhyJoinUs() {
  const cards = [
    ...
    },
    {
      ...
      icon: (
        <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.2">
          <path strokeLinecap="round" strokeLinejoin="round" d="M11 4a2 2 0 114 0v1a2 2 0 01-2 2 2 2 0 01-2-2V4zM4 7a2 2 0 114 0v1a2 2 0 01-2 2 2 2 0 01-2-2V7zM18 7a2 2 0 114 0v1a2 2 0 01-2 2 2 2 0 01-2-2V7zM4 17v-1a3 3 0 013-3h2a3 3 0 013 3v1" />
        </svg>
      ),
    },
  ];
      ...
      <div className="section-shell max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {cards.map((card) => (
            <div
              key={card.num}
              className="group rounded-3xl border border-[#EAE3D9] bg-white p-7 shadow-xs transition-all duration-300 hover:border-[#FF5F00] hover:shadow-xl hover:-translate-y-1 relative overflow-hidden"
            >
              <div className="flex items-center justify-between mb-6">
                <span className="font-mono text-xs font-extrabold text-[#FF5F00] bg-[#FFF4EC] px-3 py-1 rounded-full border border-[#FFE2CC]">
                  {card.num}
                </span>
                <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-[#FFF4EC] text-[#FF5F00] border border-[#FFE2CC] group-hover:bg-[#FF5F00] group-hover:text-white transition-colors duration-300">
                  {card.icon}
                </div>
              </div>
