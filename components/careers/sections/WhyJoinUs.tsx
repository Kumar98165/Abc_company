  ...
  return (
    <section id="why-join-us" className="bg-[#FFFBF7] py-16 lg:py-24 border-b border-[#EAE3D9]">
      <div className="section-shell max-w-7xl mx-auto px-4 sm:px-6">
        ...
        {/* 6 CARDS WITH LARGE NUMBER OUTSIDE AND BIG ICON INSIDE */}
        <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
          {cards.map((card) => (
            <div key={card.num} className="relative flex flex-col group">
              {/* LARGE NUMBER OUTSIDE / ABOVE CARD */}
              <div className="flex items-center justify-between px-2 mb-2">
                <span className="font-mono text-3xl sm:text-4xl font-black text-[#FF5F00] tracking-wider drop-shadow-2xs">
                  {card.num}
                </span>
                <span className="h-2 w-2 rounded-full bg-[#FF5F00] opacity-40 group-hover:scale-150 group-hover:opacity-100 transition-all" />
              </div>

              {/* CARD CONTAINER WITH BIG ICON INSIDE */}
              <div className="flex-1 rounded-[28px] border border-[#EAE3D9] bg-white p-7 sm:p-8 shadow-xs transition-all duration-300 hover:border-[#FF5F00] hover:shadow-xl hover:-translate-y-1 relative overflow-hidden flex flex-col justify-between">
                <div>
                  {/* BIG ICON CONTAINER INSIDE CARD */}
                  <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-[#FFF4EC] text-[#FF5F00] border border-[#FFE2CC] group-hover:bg-[#FF5F00] group-hover:text-white transition-colors duration-300 shadow-2xs mb-5">
                    {card.icon}
                  </div>

                  {/* Title & Description */}
                  <h3 className="text-xl font-extrabold text-[#0F172A] group-hover:text-[#FF5F00] transition-colors mb-2.5">
                    {card.title}
                  </h3>
